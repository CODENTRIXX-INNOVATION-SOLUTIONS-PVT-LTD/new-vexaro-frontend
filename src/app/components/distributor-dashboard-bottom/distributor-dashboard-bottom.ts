    ...
    forkJoin([transactions$, merchants$, wallet$, disputes$]).subscribe({
      next: ([txRes, merchantsRes, walletRes, disputesRes]) => {
        ...
        this.merchantActivity = txs.map(t => ({
          merchantId: t.userId?._id ?? t.userId ?? '—',
          businessName: t.userId?.companyName ?? t.userId?.firstName ?? 'Unknown',
          action: this.formatTxType(t.type),
          amount: DEBIT_TYPES.has(t.type) ? undefined : Math.abs(t.amount ?? 0),
        ...
        // totalProfit: not available from a single endpoint — show 0 until a profit report API exists
